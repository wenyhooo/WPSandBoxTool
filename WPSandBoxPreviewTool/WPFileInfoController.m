...
@interface WPFileInfoController ()<UITextViewDelegate,UIScrollViewDelegate>
...
- (void)viewDidLoad
{
    [super viewDidLoad];
    [self setUpUI];
    if ([WPSandBoxPreviewTool sharedTool].openLog) {
        NSLog(@"%@",self.filePath);
    }
}
...
- (void)addFileInfoViewWithTopView:(UIView *)topView{
    ...
    //md5
    fileMD5 = [WPFileInfo getFileMD5WithPath:self.filePath];
    self.md5 = fileMD5;
    ...
    for (int i = 0; i < infoKeyArr.count; i++) {
        ...
        if (i == 0) {
            ...
            [contentlb addGestureRecognizer:tapGesture];
        }
...
{
    if(self.tView.contentSize.height - self.tView.contentOffset.y < 100+self.tView.frame.size.height){
        self.tView.contentOffset = CGPointMake(0, 0);
        
    }else{
        self.tView.contentOffset = CGPointMake(0, self.tView.contentSize.height-self.tView.frame.size.height);
    }
}
...
{
    if(scrollView.contentSize.height - scrollView.contentOffset.y < 100+scrollView.frame.size.height){
        return YES;
    }else{
